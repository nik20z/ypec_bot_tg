...
class AnswerText:
    ...
    call_schedule = "<b>Расписание звонков</b>\n" \
                    "\n" \
                    "Понедельник - Пятница\n" \
                    "<u>" \
                    "0 |   7:40 |   8:25\n" \
    ...
    sync_code = lambda user_id_sync_code: f"ФУНКЦИЯ РАБОТАЕТ В ТЕСТОВОМ РЕЖИМЕ!\n" \
                                          f"Код синхронизации нужен для того, " \
                                          f"чтобы синхронизировать ваши настройки в Телеграм " \
                                          f"и на их основе получать рассылку (в разработке) от бота, но уже во Вконтакте\n" \
                                          f"\n" \
                                          f"Ваш персональный код:\n" \
                                          f"<code>{user_id_sync_code}</code>\n" \
                                          f"\n" \
                                          f"Линк на бота:\n" \
                                          f"{Communicate.VK_BOT}" \
                                          f""
...
class AnswerCallback:
    ...
    spam_or_subscribe_name_id = lambda action_type, result: f"{'Рассылка' if action_type == 'sp' else 'Подписка'} " \
                                                            f"{'активирована' if result else 'удалена'}"
