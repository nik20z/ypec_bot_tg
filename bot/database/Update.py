...
def user_settings_value(user_id: int, name_: str, value: str, remove_=False):
    ...
    if remove_:
        query = """UPDATE telegram 
                    ...
                    RETURNING True""".format(user_id, name_, value)
        ...
        query = """UPDATE telegram 
                    ...
                    WHERE user_id = {0}
                    RETURNING NOT {1} ISNULL""".format(user_id, name_, value)
    cursor.execute(query)
    connection.commit()
    res = cursor.fetchone()
...
def user_settings_array(user_id: int, name_: str, value: str, remove_=False, append_=False):
    ...
    if remove_:
        query = """UPDATE telegram
                    ...
                    WHERE user_id = {0}
                    RETURNING {2} = ANY({1}_ids)""".format(user_id, name_, value)
    elif append_:
        query = """UPDATE telegram
        ...
        query = """UPDATE telegram
                    SET {1}_ids = CASE 
                        ...
                        END
                    WHERE user_id = {0}
                    RETURNING {2} = ANY({1}_ids)""".format(user_id,
                                                           name_,
                                                           value,
